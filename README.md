...
一个可复用的 Skill，用于将招聘邮件、招聘官网、岗位页面和用户补充信息整理成统一、按优先级排序的求职申请流程。

这个 Skill 的愿望很简单：秋招到了、海投开始，不要再被看不过来的招聘邮件、笔试面试通知和官网状态淹没。把零散信息交给大模型汇总、去重、排序，让真正重要的下一步始终清晰可见。
...
- 从面试、笔试和截止日期中整理日程候选，核对关键信息并经确认后添加到日历或设置提醒。
...
```text
从最新招聘邮件中找出已经确认的面试和笔试，先列出日程标题、时间、时区、会议链接和提醒设置；我确认后，再添加到我的日历，并避免创建重复事件。
```
